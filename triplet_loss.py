...
if __name__=='__main__':
    ...
    x=tf.placeholder(tf.float32,[None,28,28,1],name='placeholder_x')
    y=tf.placeholder(tf.float32,[None],name='placeholder_y')
    ...
    train_op=traning(cost,global_step)
    ...
    traning_dataset=tf.data.Dataset.from_tensor_slices((features_placeholder,labels_placeholder))
    batched_dataset=traning_dataset.batch(batch_size)
    
    traning_init_op=batched_dataset.make_initializable_iterator()
    next_element = traning_init_op.get_next()
    ...
    with tf.device('/cpu:0'):
        with tf.Session() as sess:
            ...
            for epoch in range(training_epochs):
                ...
                sess.run(traning_init_op.initializer,feed_dict={features_placeholder:x_train_real,labels_placeholder:y_train})
            ...
            correct_pred=tf.equal(predict_op,tf.arg_max(labels_placeholder,1))
            
            accuracy = tf.reduce_mean(tf.cast(correct_pred, "float"))
            
            train_accuracy = accuracy.eval({x: x_train, y: y_train})
            test_accuracy = accuracy.eval({y: y_test, y: y_test})
            print ("Train Accuracy:", accuracy.eval({x: x_train, y: y_train}))
            print ("Test Accuracy:", accuracy.eval({y: y_test, Y: y_test}))
